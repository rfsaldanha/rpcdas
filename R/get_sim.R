...
get_sim <- function(
  ...
  if (!("datasus-sim" %in% list_pcdas_tables(pcdas_token = pcdas_token))) {
    stop(
      "Your token does not have access to 'datasus-sim' index. Please ask PCDaS to grant your access to this index."
    )
  }
  ...
  sql_where <- glue::glue(
    "WHERE ano_obito IN ({glue::glue_collapse(ano, sep = ', ')})"
  )
  ...
  if (agg_time == "year") {
    sql_select <- glue::glue(
      sql_select,
      ", DATETIME_FORMAT(data_obito, \'yyyy\') AS agg_time"
    )
    sql_group_by <- glue::glue(sql_group_by, ", agg_time")
  } else if (agg_time == "month") {
    sql_select <- glue::glue(
      sql_select,
      ", DATETIME_FORMAT(data_obito, \'yyyy-MM\') AS agg_time"
    )
    sql_group_by <- glue::glue(sql_group_by, ", agg_time")
  } else if (agg_time == "week") {
    sql_select <- glue::glue(
      sql_select,
      ", DATETIME_FORMAT(data_obito, \'yyyy-ww\') AS agg_time"
    )
    sql_group_by <- glue::glue(sql_group_by, ", agg_time")
  }
